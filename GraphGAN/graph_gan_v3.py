"""
GraphGAN for Crossmap V3
20180518

Add tf.summary for g and d loss
Add mr_predict for every epoch
Random initialize the embedding of D and G
...
class GraphGan(object):
    def __init__(self):
        ...
        self.n_node, self.linked_nodes = utils.read_edges(config.train_filename, config.test_filename, config.n_node)
        ...
        print("Initial the embedding randomly!")
        self.node_embed_init_d = np.random.rand(config.n_node, config.n_embed)
        self.node_embed_init_g = np.random.rand(config.n_node, config.n_embed)
        ...
        for epoch in tqdm.tqdm(range(config.max_epochs)):
            ...
            for d_epoch in tqdm.tqdm(range(config.max_epochs_dis)):
                ...
                while True:
                    ...
                    feed_dict_d = {self.discriminator.q_node: np.array(input_q_node),
                                   self.discriminator.rel_node: np.array(input_rel_node),
                                   self.discriminator.label: np.array(input_label)}
                    _, d_loss = self.sess.run([self.discriminator.d_updates, self.discriminator.pre_loss], feed_dict_d)
                ...
                for root_node in tqdm.tqdm(self.root_nodes, mininterval=3):  # random update trees
                    if np.random.rand() < config.update_ratio:
                        ...
                        if cnt % config.gen_update_iter == 0 and cnt > 0:
                            ...
                            feed_dict_g = {self.generator.q_node: np.array(q_node_gen), 
                                           self.generator.rel_node: np.array(rel_node_gen),
                                           self.generator.reward: reward_gen}
                            _, g_loss, prob = self.sess.run([self.generator.gan_updates, self.generator.gan_loss, self.generator.i_prob],
            ...
            feed_dict_s = dict(feed_dict_g.items() + feed_dict_d.items())
            ms = self.sess.run(self.summary_op, feed_dict=feed_dict_s)
            ...
            print("Mr predicting and embedding writing of Epoch%d done!\n" % epoch)
    ...
    def generate_window_pairs(self, sample_path):
        ...
        sample_path = sample_path[:-1] # eliminate the last one which is the father of the leaf
        pairs = []
    ...
    def eval_test_mr(self, epoch):
        modes = [self.generator, self.discriminator]
        print("Epoch%d: generator embedding" % epoch)
        node_embed = self.sess.run(modes[0].node_embed)
        self.mr_predict(node_embed)
        print("Epoch%d: discriminator embedding" % epoch)
        node_embed = self.sess.run(modes[1].node_embed)
        self.mr_predict(node_embed)
