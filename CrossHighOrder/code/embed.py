...
class CrossMap(object):
	...
	def fit2nodes(self, tweets, voca):
		random.shuffle(tweets)
		locations = [[tweet.lat, tweet.lng] for tweet in tweets]
		times = [[tweet.ts] for tweet in tweets]
		self.lClus.fit(locations)
		...
		nt2nodes, et2net = self.prepare_training_data(tweets, voca) # nt stands for "node type", and et stands for "edge type"
		self.node_id = self.write_graph(nt2nodes, et2net)		
		edge_type_list = ['tw', 'wl', 'tl', 'ww', 'lw', 'lt', 'wt']
		self.nt2vecs = dict()
		sample_size = len(tweets)*self.pd['epoch']
		# for tp in edge_type_list:
		# 	edge_tp = defaultdict(lambda : defaultdict(lambda : defaultdict(float)))
		# 	edge_tp[tp] = deepcopy(et2net[tp])
		# 	self.nt2vecs[tp], self.nt2cvecs = self.embed_algo.fit(nt2nodes, edge_tp, sample_size)
		...
		for key1 in et2net.keys():
			for key2 in et2net[key1].keys():
				...
				for key3 in et2net[key1][key2].keys():
					if key1[1] == 'l':
						key3_v = str(self.lClus.centroids[key3])
					elif key1[1] == 't':
						key3_v = str(self.tClus.centroids[key3])
					else:
						key3_v = key3
					line = [key1, str(node_id[key2_v]), str(node_id[key3_v]), str(et2net[key1][key2][key3]),'\n']
					# line = [str(node_id[key2_v]), str(node_id[key3_v]), '\n']
					edge_num = edge_num + 1
					gf.write(' '.join(line))
		gf.close()
		print 'There are ', edge_num, 'edges in total!'
		print et2net.keys()
		print 'll edges num is ', len(et2net['ll'].keys())
		print 'tt edges num is ', len(et2net['tt'].keys())
		print 'ww edges num is ', len(et2net['ww'].keys())				
		return node_id


	def update_vec_cvec(self, nt2vecs, nt2cvecs):
		for nt in nt2vecs.keys():
			for node in nt2vecs[nt].keys():
				self.nt2vecs[nt][node] = nt2vecs[nt][node]
				self.nt2cvecs[nt][node] = nt2cvecs[nt][node]				
