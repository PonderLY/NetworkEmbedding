"""
Add read_embedding_tf for reading embedding from GraphGAN
"""
import numpy as np
import random
...
class CrossMap(object):
	...
	def fit(self, tweets, voca):
		...
		self.node_id = self.write_graph(nt2nodes, et2net)
	...
	def write_graph(self, nt2nodes, et2net):
		nts = self.pd['nt_list']
		num = -1
		df = open(self.pd['node_dict'], 'w')
		nt = 't'
		nodelist = [str(c) for c in self.tClus.centroids]
		for j in range(len(nodelist)):
			num = num + 1
			wt_list = [str(num), nt, str(j), nodelist[j], '\n']
			df.write('\x01'.join(wt_list))
		nt = 'w'
		node_nt = list(nt2nodes[nt])
		nodelist.extend(node_nt)
		for j in range(len(node_nt)):
			num = num + 1
			wt_list = [str(num), nt, str(j), node_nt[j], '\n']
			df.write('\x01'.join(wt_list))
		nt = 'l'
		node_nt = [str(c) for c in self.lClus.centroids]
		nodelist.extend(node_nt)
		for j in range(len(node_nt)):
			num = num + 1
			wt_list = [str(num), nt, str(j), node_nt[j], '\n']
		...
		print 'There are ', node_num, 'nodes in total!'

		node_id = dict(zip(nodelist, range(0, node_num)))
		gf = open(self.pd['graph_edges'], 'w')
		for key1 in et2net.keys():
			for key2 in et2net[key1].keys():
				if key1[0]=='l':
					key2_v = str(self.lClus.centroids[key2])
				elif key1[0]=='t':
					key2_v = str(self.tClus.centroids[key2])
				else:
					key2_v = key2
				for key3 in et2net[key1][key2].keys():
					if key1[1] == 'l':
						key3 = str(self.lClus.centroids[key3])
					if key1[1] == 't':
						key3 = str(self.tClus.centroids[key3])
					# line = [str(node_id[key2]), str(node_id[key3]), str(et2net[key1][key2][key3]),'\n']
					line = [str(node_id[key2_v]), str(node_id[key3]), '\n']
					gf.write(' '.join(line))
		gf.close()
		return node_id


	def read_embedding(self, pd):
		node_f = open(pd['node_dict'], 'r')
		node_lines = node_f.readlines()
		node_id2nt = {}
		for line in node_lines:
			line_lst = line.split('\x01')
			node_id2nt[int(line_lst[0])] = [line_lst[1], line_lst[2], line_lst[3]]
		node_f.close()
		embed_f = open(pd['mr_embedding_dir'], 'r')
		embed_lines = embed_f.readlines()[1:]
		embed_f.close()
		# self.nt2vecs = dict()
		# for nt in self.pd['nt_list']:
		# 	self.nt2vecs[nt] = dict()
		for line in embed_lines:
			line_lst = line.split('\t')
			line_id = int(float(line_lst[0]))
			line_node = node_id2nt[line_id][1]
			line_type = node_id2nt[line_id][0]
			line_lst.pop(0)
			line_vec = np.array([float(i) for i in line_lst])
			if line_type == 'w':
				self.nt2vecs[line_type][node_id2nt[line_id][2]] = line_vec
			else:
				self.nt2vecs[line_type][int(line_node)] = line_vec


	def read_embedding_tf(self, config, node_embed, context_embed):
		"""
		node_embed is the embedding results of GraphGAN
		"""
		node_f = open(config.node_dict, 'r')
		node_lines = node_f.readlines()
		node_id2nt = {}
		for line in node_lines:
			line_lst = line.split('\x01')
		...
		line_id = 0
		for line in node_embed:
			line_node = node_id2nt[line_id][1]
			line_type = node_id2nt[line_id][0]
			line_vec = np.array([float(i) for i in line])
			if line_type == 'w':
				self.nt2vecs[line_type][node_id2nt[line_id][2]] = line_vec
			else:
				self.nt2vecs[line_type][int(line_node)] = line_vec
		...
		line_id = 0
		for line in context_embed:
			line_node = node_id2nt[line_id][1]
			line_type = node_id2nt[line_id][0]
			line_vec = np.array([float(i) for i in line])
			if line_type == 'w':
				self.nt2cvecs[line_type][node_id2nt[line_id][2]] = line_vec
			else:
				self.nt2cvecs[line_type][int(line_node)] = line_vec
			line_id = line_id + 1
